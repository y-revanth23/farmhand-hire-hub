...
-- 1) Roles enum
create type if not exists public.app_role as enum ('admin', 'owner', 'farmer', 'user');
...
-- 10) Trigger to auto-assign roles on signup (default user, admin for specific email, optional role from metadata)
...
begin
  ...
  -- Assign admin to the specified email
  ...
  -- If user provided an app role in metadata, add it (supports 'farmer' or 'owner')
...
-- 11) Attach triggers to auth.users
create trigger if not exists on_auth_user_created_profiles
  after insert on auth.users
  for each row execute procedure public.handle_new_user_profiles();

create trigger if not exists on_auth_user_created_roles
  after insert on auth.users
